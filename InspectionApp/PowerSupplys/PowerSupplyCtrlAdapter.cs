...
using PowersupplyCtrl;
using PowersupplyCtrl2;

namespace InspectionApp
{
    /// <summary>
    /// PowerSupplyCtrlライブラリ用のアダプター
    /// </summary>
    public class PowerSupply1CtrlAdapter : IPowerSupply
    {
        private readonly PowersupplyCtrl.PSCtrl _powerSupply;

        /// <summary>
        /// 指定した接続を使用してアダプターを初期化します
        /// </summary>
        /// <param name="connection">使用する接続インターフェース</param>
        public PowerSupply1CtrlAdapter(PowersupplyCtrl.IConnection connection)
        {
            _powerSupply = new PowersupplyCtrl.PSCtrl(connection);
        ...
        public PowerSupply1CtrlAdapter(string ipAddress, int port)
        {
            _powerSupply = new PowersupplyCtrl.PSCtrl(ipAddress, port);
            _powerSupply.ConnectionStatusChanged += OnPowerSupplyConnectionChanged;
            _powerSupply.ErrorOccurred += OnPowerSupplyErrorOccurred;
        }
        ...
        public PowerSupply1CtrlAdapter(string portName, int baudRate = 9600, Parity parity = Parity.None,
                                     int dataBits = 8, StopBits stopBits = StopBits.One)
        {
            _powerSupply = new PowersupplyCtrl.PSCtrl(portName, baudRate, parity, dataBits, stopBits);
            _powerSupply.ConnectionStatusChanged += OnPowerSupplyConnectionChanged;
            _powerSupply.ErrorOccurred += OnPowerSupplyErrorOccurred;
        }
        ...
        private void OnPowerSupplyConnectionChanged(object? sender, PowersupplyCtrl.ConnectionEventArgs e)
        {
            ConnectionStatusChanged?.Invoke(this,
                new ConnectionStatusChangedEventArgs(e.IsConnected, e.Message));
        }

        private void OnPowerSupplyErrorOccurred(object? sender, PowersupplyCtrl.PowerSupplyErrorEventArgs e)
        {
