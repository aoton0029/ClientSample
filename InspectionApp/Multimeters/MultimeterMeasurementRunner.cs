...
{
    /// <summary>
    /// IMultimeterを使用して定期的に測定を行うクラス
    /// </summary>
    public class MultimeterMeasurementRunner : IDisposable
    {
        private readonly IMultimeter _multimeter;
        ...
        public MultimeterMeasurementRunner(IMultimeter multimeter, IDataProcessor dataProcessor)
        {
            _multimeter = multimeter ?? throw new ArgumentNullException(nameof(multimeter));
            _dataProcessor = dataProcessor ?? throw new ArgumentNullException(nameof(dataProcessor));

            _multimeter.ErrorOccurred += (sender, args) =>
        ...
        private void StartMeasurementTask()
        {
            ...
            _measurementTask = Task.Run(async () =>
            {
                ...
                try
                {
                    // 最後の測定実行時間と現在の機能を格納するディクショナリ
                    var lastMeasurementTimes = new Dictionary<int, DateTime>();
                    string currentFunction = await _multimeter.GetFunctionAsync();
                    ...
                    while (!_cts.Token.IsCancellationRequested)
                    {
                        ...
                        foreach (var setting in _activeSettings.ToArray()) // ToArray()でロックなしでコピーを処理
                        {
                            try
                            {
                                ...
                                {
                                    // 機能設定が必要な場合
                                    if (!string.IsNullOrEmpty(setting.MultimeterFunction) &&
                                        currentFunction != setting.MultimeterFunction)
                                    {
                                        await _multimeter.SetFunctionAsync(setting.MultimeterFunction);
                                        currentFunction = setting.MultimeterFunction;
                                    }

                                    await PerformMeasurementAsync(setting, currentFunction);
        ...
        /// </summary>
        /// <param name="settings">測定設定</param>
        /// <param name="currentFunction">現在の測定機能</param>
        private async Task PerformMeasurementAsync(MeasurementSettings settings, string currentFunction)
        {
            ...
            switch (settings.MeasurementType)
            {
                case MeasurementType.Voltage:
                    measuredValue = await _multimeter.MeasureVoltageAsync();
                    if (string.IsNullOrEmpty(source))
                        source = "Multimeter_Voltage";
                    break;

                case MeasurementType.Current:
                    measuredValue = await _multimeter.MeasureCurrentAsync();
                    if (string.IsNullOrEmpty(source))
                        source = "Multimeter_Current";
                    break;

                case MeasurementType.MultimeterValue:
                    // 現在設定されている機能での汎用測定（具体的なAPIがないのでVoltageで代用）
                    measuredValue = await _multimeter.MeasureVoltageAsync();
                    if (string.IsNullOrEmpty(source))
                        source = $"Multimeter_{currentFunction}";
                    break;
